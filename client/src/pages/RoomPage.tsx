...
import { createRegieHistoryEntry } from "../api/regieHistory";
...
export function RoomPage({ roomId, roomName, roomCreator, currentUser, onNavigate, theme = "dark", onThemeToggle }: RoomPageProps) {
  ...
  const buildSessionPollDraftsFromCurrentVideo = useCallback((variantSeed = sessionPollGenerationCount): SessionPollDraftSuggestion[] => {
    ...
    return questionSet.map((question, index) => ({
      question,
      options: rotate(optionSet[index], variantSeed + index).slice(0, 4),
...
const handleToggleFavorite = async (videoId: string) => {
  ...
  const handleVoteSessionPoll = useCallback(async (optionId: string) => {
    ...
    const votedOption = nextPoll.options.find((option) => option.id === optionId);
        ...
        <div className={`w-80 ${theme === 'dark' ? 'bg-zinc-900' : 'bg-gray-100'} rounded-xl flex flex-col sticky top-20 h-[calc(100vh-6rem)]`}>
          ...
          {activeTab === "polls" && backendSalonId && canUsePolls && (
            <div className="flex-1 h-full overflow-y-auto p-4">
              ...
              <div className={`rounded-2xl border p-4 ${theme === "dark" ? "border-zinc-800 bg-zinc-900/80" : "border-gray-200 bg-white"}`}>
                <div>
                  <h3 className={`text-base font-semibold ${theme === "dark" ? "text-white" : "text-black"}`}>
                    Sondage régie
                  </h3>
                  <p className={`mt-1 text-sm ${theme === "dark" ? "text-zinc-400" : "text-gray-600"}`}>
                    Créez un sondage manuel ou générez-en un à partir de la vidéo en cours.
                  </p>
                </div>
                <Button
                  size="sm"
                ...
                >
                  ...
                  {canManageSessionPoll ? "Ouvrir le sondage régie" : liveSessionPoll ? "Voir le sondage" : "Sondage"}
                </Button>
              </div>
              <div className="mt-4">
                {liveSessionPoll ? (
